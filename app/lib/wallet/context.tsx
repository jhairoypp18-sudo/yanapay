...
export function WalletProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    // Re-check immediately in case wallets registered between render and effect
    handleWalletsChanged(discoverWallets());
  ...
  const connect = useCallback(async (connectorId: string) => {
    ...
    try {
      const s = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error("La wallet no respondió. ¿Abriste la ventana de Phantom?")),
            60_000
          )
        ),
      ]);
  ...
  const value = useMemo<WalletContextValue>(
    ...
    [connectors, status, session, signer, error, connect, disconnect, cancelConnect, isReady]
  );
